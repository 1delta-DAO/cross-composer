      ...
      {({ account, chain, openAccountModal, openChainModal, openConnectModal, mounted, authenticationStatus }) => {
        ...
        return (
          <div className="flex items-center gap-2" aria-hidden={!ready}>
            ...
            <button onClick={openAccountModal} type="button" className="btn btn-primary btn-sm flex items-center gap-2">
              {account?.displayBalance && <span className="text-xs font-mono">{account.displayBalance}</span>}
