...
export function buildAddressUrl(registry: ChainsRegistryRecord, chainId: string, address: string): string | undefined {
...
export function buildTokenUrl(registry: ChainsRegistryRecord, chainId: string, token: string): string | undefined {
  const chain = registry[chainId]
  const base = getPrimaryExplorerBase(chain)
  if (!base) return undefined
...
export function buildTransactionUrl(registry: ChainsRegistryRecord, chainId: string, txHash: string): string | undefined {
  const chain = registry[chainId]
  const base = getPrimaryExplorerBase(chain)
  if (!base) return undefined
