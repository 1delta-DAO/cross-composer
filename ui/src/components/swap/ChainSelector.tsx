...
export function ChainSelector({ value, onChange }: Props) {
    ...
    const allowed = React.useMemo(() => new Set(SUPPORTED_CHAIN_IDS), [])
    ...
    const relevant = React.useMemo(() => items.filter((i) => RELEVANT_CHAIN_IDS.includes(i.id as ChainEnum)).slice(0, 3), [items])
