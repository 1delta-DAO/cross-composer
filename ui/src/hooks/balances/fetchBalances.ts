...
async function multicallRetry(chainId: string, calls: Call[], abis: any[], _retries?: number): Promise<any[]> {
    const client = await getOptimizedEvmClient(chainId)
    if (!client) throw new Error(`No client available for chain ${chainId}`)
...
export async function fetchEvmUserTokenDataEnhanced(
    ...
    if (argentCall) {
        ...
        try {
            let provider = await getOptimizedEvmClient(chainId)
    ...
    const nativeData: BalanceInfo = nativeBalance
        ? {
              ...
              balance: formatUnits(nativeBalance.balance, getWNative(chainId).decimals),
              balanceUsd: 0,
          }
        : {
              balanceRaw: "0",
